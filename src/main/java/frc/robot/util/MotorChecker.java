...
public class MotorChecker {
    ...
    public static class PDPChannelChecker {
        ...
        public static class CheckerConfig {
            ...
            public CheckerConfig(double runtimeSec, double runSpeed)
            {
                this.mRunTimeSec = runtimeSec;
        ...
        public static class CheckerMotor {
            private GZSpeedController mController;
            private GZSubsystem mSubsystem;
            private CheckerConfig mConfig;
            public CheckerMotor(GZSpeedController c, GZSubsystem s, CheckerConfig config) {
                this.mController = c;
                this.mSubsystem = s;
                this.mConfig = config;
            }
            ...
            public GZSpeedController getController() {
        ...
        private ArrayList<CheckerMotor> speedControllersToTestPDPChannels;
        ...
        public void clear() {
            speedControllersToTestPDPChannels = new ArrayList<CheckerMotor>();
        }

        public void addControllers(List<CheckerMotor> motor) {
            for (CheckerMotor c : motor)
                addController(c);
        }

        public void addController(CheckerMotor motor) {
            speedControllersToTestPDPChannels.add(motor);
        }

        public void run(double waitTime) {
            for (CheckerMotor c : speedControllersToTestPDPChannels) {

                ArrayList<Integer> result = checkController(c);
                if (result.size() == 0)
                    System.out.println(c.getSubsystemName() + ": motor " + c.getController().getGZName()
                            + " PDP channels could not be determined");
                else {
                    String out = c.getSubsystemName() + ": motor PDP Channels could be: ";
        ...
        private ArrayList<Integer> checkController(CheckerMotor c) {
            ArrayList<Integer> retval = new ArrayList<Integer>();
            try {

                System.out.println("Checking " + c.getSubsystemName() + ": motor " + c.getController().getGZName());

                ArrayList<Double> pdpVals = new ArrayList<Double>();
                c.getController().lockOutController(true);

                c.getController().set(c.getConfig().getRunSpeed(), true);
                Timer.delay(c.getConfig().getRunTime());
                ...
                c.getController().lockOutController(false);
            }
            return retval;
        }
