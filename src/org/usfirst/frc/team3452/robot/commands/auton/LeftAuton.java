...
public class LeftAuton extends CommandGroup {
	...
	private void switchL(int mode) {
		if (mode == 1) {
			...
			addSequential(new DriveTime(-.25, 0, .225)); // jog forward
															// backwards to drop
															// arm

			addSequential(new EncoderFrom(7.91, 7.91, .3, .3, .4)); // drive to
																	// side of
																	// switch
			addParallel(new ElevatorPosition(3)); // raise arm

			addSequential(new EncoderFrom(0.75, -1.5, .5, .5, .5)); // turn to
																	// switch
			addSequential(new EncoderFrom(.65, .65, .5, .5, .5)); // drive and
																	// drop
